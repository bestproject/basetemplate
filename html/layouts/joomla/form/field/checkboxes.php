...
<fieldset id="<?php echo $id; ?>" class="<?php echo trim($class . ' checkboxes'); ?>"
	...
	<?php foreach ($options as $i => $option) : ?>
		<?php
			...
			$optionClass    = !empty($option->class) ? 'class="' . $option->class . ' form-check-input"' : 'class="form-check-input"';
		...
		<label for="<?php echo $oid; ?>" class="checkbox form-check-label">
            <?php echo str_ireplace(['[',']'],['<a href="'.Joomla\CMS\Router\Route::_('index.php?Itemid=135').'" target="_blank">','</a>'], $option->text); ?>
        </label>
	<?php endforeach; ?>
</fieldset>
